...
namespace alex::io::binary
{
    ...
    // todo: explore universal coders, e.g., Elias omega coding
    ...
    void        write_vec_var_u32(std::ostream& file, const std::vector<uint32_t>& values);
    void        read_vec_var_u32(std::istream& file, std::vector<uint32_t>& values);
    ...
    void write_vec_u32(std::ostream& file, const std::vector<uint32_t>& values)
    {
        write_u32(file, (uint32_t)values.size());
    ...
    void write_vec_var_u32(std::ostream& file, const std::vector<uint32_t>& values)
    {
        write_var_u32(file, (uint32_t)values.size());
        for (const auto& value : values)
            write_var_u32(file, (uint32_t)value);
    }

    void read_vec_u32(std::istream& file, std::vector<uint32_t>& values)
    {
        const uint32_t sz = read_u32(file);
        values.reserve(sz);
        for (uint32_t i = 0; i < sz; ++i)
            values.push_back(read_u32(file));
    }

    void read_vec_var_u32(std::istream& file, std::vector<uint32_t>& values)
    {
        const uint32_t sz = read_var_u32(file);
        values.reserve(sz);
        for (uint32_t i = 0; i < sz; ++i)
            values.push_back(read_var_u32(file));
    }
