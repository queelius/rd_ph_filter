...
namespace alex::collections::set
{
     ...
     *
     * The abstract data type of the frozen approximate set is denoted by FASet. The
     * representation of an frozen approximate set S* of S with a false positive rate p is denoted
     * by FASet(S, p).
     */
    template <typename T>
    class FASet
    ...
    public:
        ...
        /*
         *  Returns an approximate set of S, ASet(S, p), where S is the set of unique elements in
         *  iterable collection c.
         */
        template <Collection>
        virtual void make(const Collection& c, probability p) = 0;

        /*
         *  Returns ASet(S, p), where S is the set of unique elements in an iterable
