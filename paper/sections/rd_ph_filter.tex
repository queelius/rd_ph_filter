...
\COMMENT{
\begin{algorithm}[h]
    \caption{Implementation of $\operatorname{contains} : 2^U \times U \mapsto \{0,1\}$}
    \label{alg:PHF_has}
    \SetKwProg{func}{function}{}{}
    \KwIn
    {
        \begin{enumerate}
            \item[$s$] A rate-distorted perfect hash filter object.
            \item[$x$] The object to test membership of in $s$.
        \end{enumerate}
    }
    \KwOut
    {
        \True if $x \in A$, otherwise \True with probability $\varepsilon$ and false with probability $1 - \varepsilon$.
    }
    \func{$\operatorname{contains}(A,x)$}
    {
        $V \gets \operatorname{get_bit_matrix}(A)$\;
        $M \gets \operatorname{ncols}(V)$\;
        $N \gets \operatorname{nrows}(V)$\;
        $i \gets \ph_{A}(x)$\;
        $h \gets \operatorname{hash}(x) \mod N$\;
        \Return row $i$ of $V$ is $h$\;
    }
\end{algorithm}
}
...
The rate-distorted perfect hash filter models the concept of a Bernouilli set
