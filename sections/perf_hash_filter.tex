...
\end{equation}
where $q = 1 - \frac{(m/r)!}{(m/r)^m ((m/r)-m)!}$, $m = \Card{\Set{S}}$, and $r$ is the load factor. Since $m = N r$

It does contain some information. We know that if the size of the vector of hashes is $m$, $\Card{\Set{S}} \geq ?$
